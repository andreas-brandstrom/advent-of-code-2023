pub(crate) fn calibration_value() {
    let calibration_document = "six1mpffbnbnnlxthree
    4eight3one92
    9nine2xnhvjtjlzj48
    ...
    let sum = calculate_calibration_value(calibration_document);

    println!("Sum: {sum}");
}

fn calculate_calibration_value(calibration_document: &str) -> u32 {
    let mut first: u32 = 0;
    let mut last: u32 = 0;
    let mut sum: u32 = 0;
    let mut window = ['a'; 5];

    for char in calibration_document.chars() {
        window.rotate_left(1);
        window[window.len()-1] = char;

        let value: char = match window {
            [.., 'o', 'n', 'e'] => '1',
            [.., 't', 'w', 'o'] => '2',
            ['t', 'h', 'r', 'e', 'e'] => '3',
            [.., 'f', 'o', 'u', 'r'] => '4',
