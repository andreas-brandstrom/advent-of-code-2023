...
impl AdjacentArea {
    fn is_adjacent(&self, coordinate: &Coordinate ) -> bool {
        let inside_x_coordinates = coordinate.0 >= self.top_left_corner.0 &&
            coordinate.0 <= self.bottom_right_corner.0;
        let inside_y_coordinates = coordinate.1 >= self.top_left_corner.1 &&
            coordinate.1 <= self.bottom_right_corner.1;
        
        inside_x_coordinates && inside_y_coordinates
    }
    
}

impl PartNumber {
    fn create(value: u32, coordinate: &Coordinate) -> Self {
        ...
        let adjacent_area = AdjacentArea{
...
pub(crate) fn schematic_sum() {
    ...
    let (parts_list, symbol_coordinates, gear_coordinates) = parse_schematic(schematic);

    let mut schematic_sum:u32 = 0;
    'parts: for part in &parts_list {
        for symbol in &symbol_coordinates { 
            if part.adjacent_area.is_adjacent(symbol) {
    ...
    for gear in &gear_coordinates {
        let mut adjecent_parts:Vec<&PartNumber> = vec![];
        for part in &parts_list {
            if part.adjacent_area.is_adjacent(gear) {
                adjecent_parts.push(part);
            }
        }

        if adjecent_parts.len() == 2 {
            let first_gear = adjecent_parts.pop();
            let second_gear = adjecent_parts.pop();

            if let (Some(part_1),Some(part_2)) = (first_gear, second_gear) {
                gear_ratio_sum += part_1.value*part_2.value;
            }
        }
    }

    println!("Schematic Sum: {schematic_sum}");
    println!("Gear Ratio Sum: {gear_ratio_sum}");
}

fn parse_schematic(schematic: &str) -> (Vec<PartNumber>, Vec<Coordinate>, Vec<Coordinate>){
    let mut parts_list:Vec<PartNumber> = vec![];
    let mut symbol_coordinates:Vec<Coordinate> = vec![];
    let mut gear_coordinates:Vec<Coordinate> = vec![];

    let mut window = [' '; 10];
    let mut x_coordinate:u32 = 0;
    let mut y_coordinate:u32 = 0;

    for char in schematic.chars() {
        ...
        match char {
            ...
            '\u{003a}'..='\u{0040}'
            => {
                let coordinate = Coordinate(x_coordinate, y_coordinate);
                symbol_coordinates.push(coordinate);

                if char == '*' {
                    gear_coordinates.push(coordinate);
                }
            },
            '.' | ' ' |'\n' | '0'..='9' => (),
            unkown => println!("{unkown:?}")
        }
        ...
        match char {
            ...
            '\u{0021}'..='\u{0039}' | '\u{003a}'..='\u{0040}' => x_coordinate += 1,
            _ => ()
        };      
    };
    (parts_list, symbol_coordinates, gear_coordinates)
}

#[cfg(test)]
mod tests {
    use crate::day_3::AdjacentArea;

    use super::{Coordinate, PartNumber};

    #[test]
    fn should_create_parts_nr() {
        let value = 120;
        let coordinate = Coordinate(113,2);

        let parts_nr = PartNumber::create(value, &coordinate);

        assert_eq!(parts_nr.value, value);

        let expected_adjecent_area = AdjacentArea{
            top_left_corner: Coordinate(110,1),
            bottom_right_corner: Coordinate(114,3)
        };

        println!("{parts_nr:?}");
        assert_eq!(parts_nr.adjacent_area, expected_adjecent_area)
    }
