...
pub(crate) fn impossible_games(){
    ...
    for char in games_data.chars() {
        ...
        match window {
            ['G','a','m','e',' ', rest @ ..] => {
                game_nr = match rest {
                    ['0'..='9',..] => {
                        let (first, _) = rest.split_at(3);
                        extract_decimal_number(first)
                    },
                    _ => 0
                };
            },
            [rest @ ..,' ','r','e','d'] => {
                let value = extract_nr_of_a_color(&rest);
                if value > max_red {
                    max_red = value
                };
                game_possible &= value <= red;
            },
            [rest @ ..,' ','g','r','e','e','n'] => {
                let value = extract_nr_of_a_color(&rest);
                if value > max_green {
    ...
    println!("Game sum: {game_sum}, Power sum:  {power_sum}");
}

fn extract_nr_of_a_color(rest: &[char]) -> u32 {
    match rest {
        [..,'0'..='9'] => {
