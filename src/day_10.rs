...
impl Move {
    fn next_position(&self, position: &(usize, usize)) -> Option<(usize, usize)> {
        match self {
            Move::Up => position.0.checked_sub(1).map(|x| (x, position.1)),
            Move::Down => position.0.checked_add(1).map(|x| (x, position.1)),
            Move::Left => position.1.checked_sub(1).map(|y| (position.0, y)),
            Move::Right => position.1.checked_add(1).map(|y| (position.0, y)),
        }
    }
    ...
    fn next_move(&self, tile: &char) -> Option<Self> {
        match (self, tile) {
            ...
            (Move::Up, '7') => Some(Move::Left),
            (Move::Down, 'L') => Some(Move::Right),
            (Move::Down, 'J') => Some(Move::Left),
            (Move::Left, 'L') => Some(Move::Up),
            (Move::Left, 'F') => Some(Move::Down),
...
pub(crate) fn pipe_maze() {
    ...
    while moves[0].1 != moves[1].1 {
        let next_moves:Vec<(Move,(usize,usize))> = moves.iter()
            .filter_map(|(m, coord)| m.next_position(coord).map(|coord| (m, coord)))
            .filter_map(|(m, coord)| maze.get(coord).map(|tile| (m, tile, coord)))
            .filter_map(|(&m, tile, coord)| m.next_move(tile).map(|m| (m,coord)))
            .collect();
...
fn initial_moves(maze: na::Matrix<char, na::Const<140>, na::Const<140>, na::ArrayStorage<char, 140, 140>>, starting_position: &(usize, usize)) -> [(Move,(usize,usize));2] {
    let initial_moves:Vec<(Move,(usize,usize))> = [Move::Up, Move::Down, Move::Left, Move::Right].iter()
        .filter_map(|m| m.next_position(starting_position).map(|coord| (m, coord)))
        .filter_map(|(m, coord)| maze.get(coord).map(|tile| (m, tile, coord)))
        .filter_map(|(&m, tile, coord)| m.next_move(tile).map(|m| (m,coord)))
        .collect();
